// src/components/ShoppingCartUseReducer.jsx
  ...
  switch (action.type) {
    ...
    case "UPDATE_QTY":
      return {
        ...state,
        cart: state.cart
          .map((p) => (p.id === action.id ? { ...p, qty: action.qty } : p))
          .filter((p) => p.qty > 0),
      };
    case "CLEAR_CART":
      return { ...state, cart: [], appliedCoupon: null, coupon: "" };
    case "SET_COUPON":
...
export default function ShoppingCartUseReducer() {
  ...
  return (
    <div className="bg-white mx-auto mt-20  shadow-2xl rounded p-4 w-full md:w-[480px]">
      <h3 className="text-lg font-semibold mb-3">🧭 useReducer Cart</h3>
      ...
      <div className="grid grid-cols-3 gap-2 mb-4">
        {SAMPLE_ITEMS.map((it) => (
          <button
            key={it.id}
            onClick={() => addItem(it)}
            className="col-span-1 bg-green-50 hover:bg-green-100 border rounded p-2 text-sm flex flex-col items-start"
          >
            ...
            {state.cart.map((item) => (
              <li key={item.id} className="py-2 flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button onClick={() => removeItem(item.id)} className="ml-3 text-sm text-red-600">
                    Remove
                  </button>
                </div>
              </li>
      ...
      <div className="mt-4">
        <div className="flex gap-2">
          ...
          />
          <button onClick={applyCoupon} className="px-3 py-2 bg-green-600 text-white rounded">
            Apply
          </button>
        </div>
        <div className="mt-2 text-sm">
          {state.appliedCoupon === "SAVE10" && <span className="text-green-600">Coupon applied: 10% off</span>}
          {state.appliedCoupon === "INVALID" && <span className="text-red-600">Invalid coupon</span>}
        </div>
      </div>
